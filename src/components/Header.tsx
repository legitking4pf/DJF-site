...
export default function RefinedHeader() {
  ...
  // 2. Track scroll state for immediate CSS logic
  ...
  // Dynamic text color logic: Dark when at top (on light image), White when scrolled
  const activeTextColor = isScrolled ? "text-white" : "text-stone-900";
  const activeSubtextColor = isScrolled ? "text-white/60" : "text-stone-500";
  const activeBorderColor = isScrolled ? "border-white/10" : "border-stone-900/10";
        ...
        <div className="max-w-[1600px] mx-auto px-6 md:px-12 h-full flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="leading-tight hidden sm:block">
              <div className={`text-sm tracking-widest font-bold transition-colors duration-300 ${activeTextColor}`}>
                JACKSON <span className="text-amber-600">F.</span>
              </div>
              <div className={`text-[9px] uppercase tracking-[0.2em] transition-colors duration-300 ${activeSubtextColor}`}>
          ...
          <nav className="lg:flex gap-10">
            {NAV_ITEMS.map((item) => (
              <a
                key={item.id}
                href={`#${item.id}`}
                className={`relative group transition-colors duration-300 ${isScrolled ? "text-white/70 hover:text-white" : "text-stone-600 hover:text-stone-900"}`}
              >
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className={`w-11 h-11 rounded-full border flex items-center justify-center transition-all duration-300 
                ${activeBorderColor} ${activeTextColor} hover:scale-105 active:scale-95`}
            >
              <Menu size={20} />
            </button>
          </div>
          ...
          >
            <div className="max-w-[1600px] mx-auto px-6 md:px-12 py-8 w-full h-full flex flex-col">
              ...
              <div className="flex items-center justify-between mb-16">
                <div className="flex items-center gap-4">
                  <div className="w-11 h-11 rounded-full bg-white text-black flex items-center justify-center font-bold">DJF</div>
                  <span className="text-sm tracking-widest uppercase font-medium">Menu</span>
                </div>
                <button
              ...
              <nav className="flex flex-col gap-8">
                {NAV_ITEMS.map((item, idx) => (
                  ...
                  >
                    <span className="text-stone-600 font-mono text-sm">0{idx + 1}</span>
